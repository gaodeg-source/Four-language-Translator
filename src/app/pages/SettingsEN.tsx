...
export function SettingsEN() {
  ...
  const hasToneChanges = Boolean(chatData) && (
    ...
    isFormal !== (chatData?.isFormal ?? false) ||
  ...
  const handleSaveTone = async () => {
    ...
    autoSave(updated, 'currentChatEN');
    navigate(`/chat-en/${updated.id}`, { replace: true });
  };
          ...
          <div className="space-y-3" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 1rem)' }}>
            {VOICES.map((v) => (
              ...
              >
                ...
                {v.id && (
                  <button onClick={(e) => { void handleDemo(v.id, e); }} className="p-2 flex-shrink-0" disabled={playingId === v.id}>
